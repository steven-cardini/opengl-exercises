...
///////////////////////////////////////////////////////////////////////////////////////////////////
{
  ...
  // draw planet 1 ///////////////////////////////////////////////////////////
    model = glm::rotate(model, P1_ANGLE, glm::vec3(0.0f, 0.0f, 1.0f)); // rotate around origin (= sun)
    model = glm::translate(model, glm::vec3(3.0f, 0.0f, 0.0f));
    model = glm::rotate(model, P1_ROTATION, glm::vec3(0.0f, 0.0f, 1.0f)); // rotate around own axis
    glUniformMatrix4fv(MV_MAT4_LOCATION, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // draw moon of planet 1
    model = glm::translate(model, glm::vec3(0.8f, 0.3f, 0.0f));
    //model = glm::rotate(model, M1_ROTATION, glm::vec3(-0.3f, 0.8f, 1.0f)); // rotation around own axis
    glUniformMatrix4fv(MV_MAT4_LOCATION, 1, GL_FALSE, glm::value_ptr(model));
    glUniform3f(COLOR_VEC3_LOCATION, 0.0f, 1.0f, 0.0f); // green
    glutSolidSphere(0.2, 20, 10);  // strictly this GLUT call is not OpenGL core compatible

  model = model_view_stack.top();

  // draw planet 2 ///////////////////////////////////////////
  model = glm::rotate(model, P2_ANGLE, glm::vec3(1.0f, 0.0f, 1.0f)); // rotate about sun
  model = glm::translate(model, glm::vec3(-5.0f, 0.0f, 0.0f));
  model = glm::rotate(model, P2_ROTATION, glm::vec3(1.0f, 0.0f, 1.0f));
  glUniformMatrix4fv(MV_MAT4_LOCATION, 1, GL_FALSE, glm::value_ptr(model));
  glUniform3f(COLOR_VEC3_LOCATION, 1.0f, 0.0f, 1.0f); 
  glutSolidSphere(0.8, 20, 10);  // strictly this GLUT call is not OpenGL core compatible
